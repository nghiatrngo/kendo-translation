...
export function calculateFuzzyScore(source: string, target: string): number {
    ...
    // Calculate n-gram similarity (for Japanese text without spaces)
...
export async function searchTM(
    ...
    try {
        ...
        // Get candidates (limit to reasonable number for fuzzy matching)
...
/**
 * Find exact or near-exact matches (100% or 95%+)
 */

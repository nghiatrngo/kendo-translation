...
// Built-in Kendo terminology (fallback if DB is empty)
const KENDO_TERMINOLOGY: Omit<TermEntry, 'id'>[] = [
    // Equipment - DO NOT TRANSLATE (keep as romaji)
    ...
    // Techniques - Required translations
...
/**
 * Search for terms in text and return matching terminology
 */
export async function searchTerminology(
    ...
    // Merge with built-in Kendo terms
    const allTerms: TermEntry[] = [
        ...dbTerms,
        ...KENDO_TERMINOLOGY.map((t, i) => ({ ...t, id: `builtin-${i}` })),
    ];

    // Remove duplicates (prefer DB entries)
    ...
    // Calculate coverage
    const coverage = foundTerms.length / Math.max(termMap.size, 1);

    return {
        foundTerms,
        constraints,
        coverage,
        missingTerms: [], // Could implement gap detection here
    };
}

/**
 * Format terminology constraints for LLM prompt
 */
...
/**
 * Validate translation against terminology constraints
 */

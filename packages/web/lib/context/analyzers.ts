...
export function analyzeDomain(text: string, lang: SourceLanguage): DomainClassification {
    ...
    // Determine secondary domain
    let secondary: DomainType | undefined;
    if (primary === 'kendo' && martialArtsScore >= 2) {
        secondary = 'martial_arts';
    } else if (primary === 'martial_arts' && kendoScore >= 2) {
        secondary = 'kendo';
    }
    ...
    return {
        ...
        secondary,
        confidence,
        indicators: indicators.slice(0, 10), // Limit to top 10
    };
}
...
export function analyzeStyle(text: string, lang: SourceLanguage): StyleProfile {
    ...
    if (lang === 'ja') {
        ...
        for (const pattern of KEIGO_PATTERNS.sonkeigo) {
            if (pattern.test(text)) sonkeigoCount++;
        }
        for (const pattern of KEIGO_PATTERNS.teineigo) {
            if (pattern.test(text)) teineigoCount++;
        }
        for (const pattern of KEIGO_PATTERNS.kenjogo) {
            if (pattern.test(text)) kenjogoCount++;
        }
        for (const pattern of KEIGO_PATTERNS.casual) {
            if (pattern.test(text)) casualCount++;
        }

        // Determine formality based on keigo
    ...
    // Determine audience based on content complexity
...
export function extractEntities(text: string, lang: SourceLanguage): Entity[] {
    ...
    if (lang === 'ja') {
        ...
        // For English, look for known translations
        const lowerText = text.toLowerCase();
        for (const [jaText, info] of Object.entries(KENDO_TERMS)) {
            const enTerm = info.translation.toLowerCase().split('/')[0]; // Take first translation option
...
export function extractKeyTerms(text: string, lang: SourceLanguage): string[] {
    ...
    if (lang === 'ja') {
        // Extract Kendo terms that appear in text
        for (const term of Object.keys(KENDO_TERMS)) {
            if (text.includes(term)) {
                terms.push(term);
            }
        ...
        // Also look for terms ending with common suffixes
        const suffixPatterns = [
            /(\S+技)/g,    // techniques ending in 技
            /(\S+法)/g,    // methods ending in 法
            /(\S+道)/g,    // ways ending in 道
        ];
        ...
        for (const pattern of suffixPatterns) {
            ...
            if (matches) {
                for (const match of matches) {
                    if (!terms.includes(match) && match.length <= 6) {
                        terms.push(match);
                    }
                }
            }
...
export function estimateComplexity(
    ...
    // Add complexity for nested structures (Japanese)
    if (lang === 'ja') {

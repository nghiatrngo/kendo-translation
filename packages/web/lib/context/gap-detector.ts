...
/**
 * Detect terminology gaps - entities without translations
 */
function detectTerminologyGaps(
    entities: Entity[],
    terminology: TerminologyConstraints
): CoverageGap[] {
...
/**
 * Detect TM coverage gaps
 */
function detectTMGaps(
    context: ContextObject,
    tmMatches: TMMatch[]
): CoverageGap[] {
    const gaps: CoverageGap[] = [];

    // No TM matches at all
    ...
    // Complex text with few examples
...
function detectDomainGaps(
    ...
    // TM matches from different domain
    const domainMismatches = tmMatches.filter(
        m => m.domain && m.domain !== context.domain.primary
    );
...
function detectStyleGaps(context: ContextObject): CoverageGap[] {
    ...
    // Instructional content without clear audience
...
function identifyStrengths(
    ...
    if (totalTerms >= 5) {
        strengths.push(`Good terminology coverage (${totalTerms} terms)`);
    }

    if (context.entities.length > 0 &&
        context.entities.every(e => e.translation || e.confidence >= 0.9)) {
...
function generateRecommendations(gaps: CoverageGap[]): string[] {
    ...
    if (highSeverity.length > 0) {
        recommendations.push('Address high-severity gaps before proceeding');
    }
    ...
    if (gaps.length === 0) {
        recommendations.push('Context is well-covered - proceed with confidence');
    }
...
/**
 * Quick gap check - returns true if critical gaps exist
 */

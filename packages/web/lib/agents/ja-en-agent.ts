...
// Verb endings that indicate subject
...
// Keigo patterns for level detection
...
// Common onomatopoeia in Kendo context
...
export function detectKeigoLevel(text: string): HonorificAnalysis {
    ...
    for (const { pattern, description } of KEIGO_PATTERNS.sonkeigo) {
        ...
        if (matches) {
            ...
            transformations.push({
                original: matches[0],
                transformed: `[respectful: ${description}]`,
                type: 'sonkeigo',
            });
        }
    }
    ...
    for (const { pattern, description } of KEIGO_PATTERNS.kenjogo) {
        ...
        if (matches) {
            ...
            transformations.push({
                original: matches[0],
                transformed: `[humble: ${description}]`,
                type: 'kenjogo',
            });
        }
    }
    ...
    if (sonkeigoCount > 0 || kenjogoCount > 0) {
        level = sonkeigoCount > kenjogoCount ? 'sonkeigo' :
            kenjogoCount > sonkeigoCount ? 'kenjogo' : 'mixed';
...
/**
 * Infer subjects from verb forms in Japanese text
 */
    ...
    for (const segment of segments) {
        ...
        for (const { pattern, subject, confidence, source } of VERB_SUBJECT_PATTERNS) {
            ...
            if (match) {
                ...
                break; // One inference per segment
            }
        }
    }
...
/**
 * Analyze sentence structure for SOV→SVO transformation needs
 */
export function analyzeStructure(text: string): StructureAnalysis {
    ...
    if (isLongSentence) {
        suggestions.push('Consider splitting into multiple sentences');
    }
    if (hasNestedClauses) {
        suggestions.push('Multiple particles detected - clarify relationships');
    }
    if (hasRelativeClauses) {
        suggestions.push('Contains embedded clauses - may need reordering');
    }
...
/**
 * Detect and suggest handling for onomatopoeia
 */
...
/**
 * Generate translation guidance based on JA analysis
 */

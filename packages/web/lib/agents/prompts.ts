...
export interface PromptTemplate {
    id?: string;
    agentType: string;
    approach?: string; // 'literal', 'natural', 'formal'
    template: string;
}
...
/**
 * Invalidate cache (e.g., after update)
 */
export function invalidatePromptCache(agentType: string, approach?: string) {
    const cacheKey = approach ? `${agentType}:${approach}` : agentType;
